# test/policies/camera_make_policy_test.rb
require "test_helper"

class CameraMakePolicyTest < ActiveSupport::TestCase
  setup do
    @admin = users(:one)
    @member = users(:two)
    @record = camera_makes(:one)
  end
  ...
  test "admin is permitted for all actions" do
    ...
    scope = Pundit.policy_scope(@admin, CameraMake)
    assert_equal CameraMake.count, scope.count
  end
  ...
  test "member is denied for all actions and scope is empty" do
    ...
    scope = Pundit.policy_scope(@member, CameraMake)
  ...
  test "guest is denied for all actions and scope is empty" do
    ...
    scope = Pundit.policy_scope(nil, CameraMake)
    assert_equal 0, scope.count
  end
end