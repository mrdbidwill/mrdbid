...
class MushroomProjectPolicyTest < ActiveSupport::TestCase
  setup do
    ...
    @mushroom_project.project = @project
    @mushroom_project.mushroom = @mushroom

    @other_project = Project.new(user_id: @other_user.id, name: "Other Project")
    @other_mushroom_project = MushroomProject.new
    @other_mushroom_project.project = @other_project
  end
  ...
  test "should authorize any signed-in user to create" do
    assert Pundit.policy(@user, MushroomProject.new).create?
    assert Pundit.policy(@other_user, MushroomProject.new).create?
  end

  test "should not authorize nil user to create" do
    assert_not Pundit.policy(nil, MushroomProject.new).create?
  end
